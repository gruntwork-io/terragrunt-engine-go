...
func TestInitResponseOneof(t *testing.T) {
	...
	assert.Equal(t, proto.LogLevel_LOG_LEVEL_INFO, logMsg.Level, "Expected correct log level")
	assert.Equal(t, "Initialization in progress", logMsg.Content, "Expected correct log content")
	...
	assert.Equal(t, int32(0), exitResult.Code, "Expected correct exit code")
	assert.Nil(t, resp.GetStdout(), "Expected stdout to be nil")
	assert.Nil(t, resp.GetStderr(), "Expected stderr to be nil")
	assert.Nil(t, resp.GetLog(), "Expected log to be nil")
}
...
func TestRunResponseOneof(t *testing.T) {
	...
	assert.Nil(t, err, "Expected no error calling Run")
	...
	assert.Equal(t, "Warning message\n", stderr.Content, "Expected correct stderr content")
	assert.Nil(t, resp.GetStdout(), "Expected stdout to be nil")
	assert.Nil(t, resp.GetExitResult(), "Expected exit result to be nil")
	assert.Nil(t, resp.GetLog(), "Expected log to be nil")
...
func TestShutdownResponseOneof(t *testing.T) {
	...
	assert.Nil(t, err, "Expected no error calling Shutdown")
	...
	for i, expectedLevel := range expectedLevels {
		resp, err := stream.Recv()
		assert.Nil(t, err, "Expected no error receiving response %d", i)

		logMsg := resp.GetLog()
		assert.NotNil(t, logMsg, "Expected log message %d", i)
		assert.Equal(t, expectedLevel, logMsg.Level, "Expected correct log level for message %d", i)
		assert.Contains(t, logMsg.Content, "Shutdown message with level", "Expected log content to contain expected text")
		assert.Nil(t, resp.GetStdout(), "Expected stdout to be nil")
		assert.Nil(t, resp.GetStderr(), "Expected stderr to be nil")
		assert.Nil(t, resp.GetExitResult(), "Expected exit result to be nil")
	}
